...
function GoalCard({ goal, period, onSetTarget, onToggle, theme: t }) {
  ...
  return (
    <div style={{ background: goal.achieved ? "rgba(0,200,150,0.06)" : "var(--bg-subtle)", border: `1px solid ${goal.achieved ? `rgba(0,200,150,0.25)` : isAtRisk ? "rgba(239,68,68,0.25)" : "var(--border-subtle)"}`, borderRadius: 12, padding: "14px 16px", transition: "all 0.2s" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 14 }}>
            {goal.achieved ? "✅" : isAtRisk ? "⚠️" : isAlmost ? "🔥" : "🎯"}
          </span>
          <span style={{ fontSize: 12, fontWeight: 500, color: "var(--text)" }}>{goal.label}</span>
        </div>
      ...
      <div style={{ marginBottom: 8 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 5, fontSize: 11 }}>
          <span style={{ color: barColor, fontFamily: "var(--font-mono)", fontWeight: 600 }}>
          ...
          <span style={{ color: "var(--text-dim)" }}>
            target: {editing ? (
              <input
                ...
                style={{ background: "transparent", border: "none", borderBottom: "1px solid var(--accent)", color: "var(--accent)", fontFamily: "var(--font-mono)", fontSize: 11, width: 60, outline: "none", padding: 0 }}
              />
            ) : (
              <span onClick={() => { setInput(goal.target.toString()); setEditing(true); }}
                style={{ color: "var(--text-dim)", cursor: "pointer", textDecoration: "underline", textDecorationStyle: "dotted" }}>
      ...
      {/* Status message */}
      {goal.achieved && <div style={{ fontSize: 10, color: "var(--accent)" }}>✓ Goal tercapai!</div>}
      {isAlmost && <div style={{ fontSize: 10, color: "var(--warning)" }}>🔥 Hampir tercapai — {(100 - goal.pct).toFixed(0)}% lagi!</div>}
      {isAtRisk && <div style={{ fontSize: 10, color: "var(--danger)" }}>⚠️ Drawdown mendekati batas!</div>}
    </div>
  );
}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 6, maxHeight: 200, overflowY: "auto" }}>
      {[...history].reverse().map((h, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: "rgba(0,200,150,0.06)", border: "1px solid rgba(0,200,150,0.15)", borderRadius: 8 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 11, color: "var(--text)", fontWeight: 500 }}>{h.label}</div>
            <div style={{ fontSize: 10, color: "var(--text-dim)" }}>{h.period === "monthly" ? "Bulanan" : "Tahunan"} · {new Date(h.achievedAt).toLocaleDateString("id-ID")}</div>
          </div>
          <div style={{ fontSize: 11, color: "var(--accent)", fontFamily: "var(--font-mono)" }}>✓</div>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div style={{ marginTop: 24, paddingTop: 24, borderTop: "1px solid var(--border-subtle)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 9, color: "var(--accent)", textTransform: "uppercase", letterSpacing: "0.2em", fontWeight: 600 }}>Goal Tracker</div>
        ...
        <div style={{ display: "flex", gap: 4, background: "var(--bg-subtle)", border: "1px solid var(--border)", borderRadius: 8, padding: 3 }}>
          {[{ v: "monthly", l: `📅 ${thisMonth}` }, { v: "yearly", l: `📊 ${thisYear}` }].map(p => (
            <button key={p.v} onClick={() => setActivePeriod(p.v)}
              style={{ padding: "5px 14px", borderRadius: 6, border: "none", cursor: "pointer", fontSize: 11, background: activePeriod === p.v ? "var(--accent)" : "transparent", color: activePeriod === p.v ? "#090e1a" : "var(--text-dim)", fontWeight: activePeriod === p.v ? 600 : 400 }}>
              {p.l}
            </button>
          ))}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ fontSize: 12, color: achievedCount === totalEnabled ? "#00c896" : "var(--text-dim)" }}>
      ...
      {!showHistory ? (
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 10 }}>
          ...
          {activeGoals.length === 0 && (
            <div style={{ gridColumn: "span 2", textAlign: "center", padding: "24px 0", color: "var(--text-dim)", fontSize: 12 }}>
      ...
      {!showHistory && activeStats.trades > 0 && (
        <div style={{ marginTop: 14, padding: "12px 16px", background: "var(--bg-subtle)", borderRadius: 10, display: "flex", gap: 20, flexWrap: "wrap" }}>
          {[
            ...
            <div key={s.label} style={{ textAlign: "center" }}>
              <div style={{ fontSize: 9, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: "0.1em" }}>{s.label}</div>
              <div style={{ fontFamily: "var(--font-mono)", fontSize: 14, color: "var(--text)", fontWeight: 600 }}>{s.value}</div>
            </div>
          ))}
        </div>
      )}
